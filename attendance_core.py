...
# ---------------- main entry ----------------
def process(roster_bytes, l2_bytes, zip_bytes, mode='exact'):
    """Returns (output_xlsx_bytes, report_rows, warnings)."""
    wb = load_workbook(io.BytesIO(roster_bytes))         # keep formulas
    wid_map = parse_l2(l2_bytes)
    ...
    chosen = {}                                          # (wid,ymd) -> member (prefer non "(1)")
    for info in z.namelist():
        pf = _parse_filename(info.split('/')[-1])
        if not pf: continue
        if pf in chosen and '(1)' in info: continue
        chosen[pf] = info
